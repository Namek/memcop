...
public abstract class Assembly {
    ...
    public static final OpDefinition[] opdefs = new OpDefinition[] {
        ...
        op("cmp", $Value, $Value, $OutRegister),
        op("ldb", $OutRegister).describe("load one source byte to specified register"),
        op("stb", $Register, $Value).describe("store byte from given register to given index inside destination memory"),
        ...
        op("je", $Value, $Label).describe("jump if equal to"),
        op("jne", $Value, $Label).describe("jump if not equal to"),
        op("jg", $Value, $Label).describe("jump if greater than"),
        op("jl", $Value, $Label).describe("jump if less than"),
        op("jge", $Value, $Label).describe("jump if greater or equal to"),
        op("jle", $Value, $Label).describe("jump if lesser or equal to"),
    };
